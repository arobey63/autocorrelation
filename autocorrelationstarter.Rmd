...
This document contains the essential functions for replicating the Mathematica code used to generate autocorrelated time series, population dynamics, and extinction risk predictions in Robey & Vasseur (2025).
...
W <- rnorm (2000, mean = 26, sd = 3)
X <- SpecSynFourier(gamma = 2, Nobs = 2000)
Z <- Mimic1(W,X)
result <- estimate_spectral_exponent(Z)
spectralexponent <- estimate_spectral_exponent(Z)$exponent
```

Plot the generated time series and its power spectra (shown here for very high autocorrelation, gamma = 2):
...
Now that you have a time series, you can define a TPC and simulate the resulting population dynamics. I've provided the generalist TPC shown in Fig 2 here (the specialist TPC instead uses 'gain1' and 'TPC_special').
```{r pop dynamics}
# TPCs
TI <- 29;
beta <- 180;
...
To assess how varying the level of temporal autocorrelation affects other TPCs for specific time series, you'll need to import the climate and species-specific TPC data. Here is one example species:
```{r example species}
# Import TPC data
...
species_data <- as.numeric(species_row[1, 2:ncol(species_row)])
...
nt <- function(t, ntlast, Z, Topt, sigmaP, CTmax, alpha) {
  T <- Z[t]
  r_val <- GQtpc(T, Topt, sigmaP, CTmax)
  numerator <- r_val * exp(r_val * delta_t)
  denominator <- (r_val / ntlast - alpha) + alpha * exp(r_val * delta_t)
